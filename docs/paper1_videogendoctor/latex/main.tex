...
% VideoGenDoctor manuscript -- compressed main-text version
% Generated from the provided TeX source and synchronized with the current paper data.
% Important: All numeric values retained from the original TeX are existing values.
% Legacy placeholders are retained only where no authenticated value has been supplied.
...
\definecolor{VDGBlue}{HTML}{1F4E79}
\definecolor{VDGGray}{HTML}{555555}
...
\titleformat{\section}
  {\Large\bfseries\color{VDGBlue}}
  {\thesection}{0.75em}{}
\titleformat{\subsection}
  {\large\bfseries}
  {\thesubsection}{0.65em}{}
\titlespacing*{\section}{0pt}{1.8ex plus .6ex minus .2ex}{0.9ex}
\titlespacing*{\subsection}{0pt}{1.3ex plus .4ex minus .2ex}{0.55ex}
...
\AtBeginEnvironment{table}{\centering\small\setlength{\tabcolsep}{5.5pt}\renewcommand{\arraystretch}{1.12}}

\pretitle{\begin{center}\LARGE\bfseries\color{VDGBlue}\linespread{1.05}\selectfont}
\posttitle{\par\end{center}\vspace{0.35em}}
\preauthor{\begin{center}\normalsize\color{VDGGray}}
...
\newcommand{\TBD}{\textcolor{TodoRed}{\textbf{TBD}}}
\newcommand{\todoresult}[1]{\textcolor{TodoRed}{\textbf{TBD: #1}}}
...
% Legacy evaluation placeholder macros retained for compatibility with earlier source structure.
...
\title{VideoGenDoctor: Structured Failure Diagnosis, Evidence Localization,\\and Repair Planning for Controllable Video Generation}
...
\begin{abstract}
Controllable video generation increasingly relies on structured conditions such as character identity, camera trajectory, required props, temporal actions, and multi-shot specifications. Existing evaluation pipelines mostly return scalar quality or alignment scores, which are insufficient for debugging because they rarely identify what failed, where it failed, which evidence supports the diagnosis, or how the next generation attempt should be repaired.

We present VideoGenDoctor, a structured framework for failure diagnosis, evidence localization, and repair planning. Given a generated video and its prompt or structured specification, VideoGenDoctor converts failures into code--span--target--plan records grounded in temporal evidence and representative keyframes. The default implementation combines lightweight feature-based screening, optional VLM verification, and a patch compiler that maps localized failures to repair plans.

We position VideoGenDoctor-Bench-v0 as a controlled diagnostic fixture rather than a broad open-world benchmark. It contains 324 videos, 9 deterministic perturbation types, and 2,016 human-verified evidence spans covering 12 observed failure codes from a 38-code taxonomy. On this fixture, VideoGenDoctor-diagnosis reaches macro-F1 0.9110 and tIoU@0.5 0.7316; Rule+GPT-4V reaches 0.9276 and 0.7688. In automatic closed-loop evaluation, VideoGenDoctor-full improves Pass@1/2 from 24.07\%/35.80\% for Score-only feedback to 64.81\%/83.64\%. On a 240-video real-failure subset from four controllable video generators, the default VideoGenDoctor pipeline improves front-end diagnosis macro-F1 from 0.7992 to 0.8264 and tIoU@0.5 from 0.5431 to 0.5982 over direct structured VLM reporting, while also improving downstream repair utility. These results support the value of structured, auditable repair records while leaving broad open-world generality as future validation.
\end{abstract}
...
Recent diffusion-based models can synthesize multi-second videos conditioned on text, reference images, camera trajectories, character identities, and structured shot scripts~\cite{cogvideox2024,svd2023,motionctrl2024,instructvideo2024}. In production-style workflows, ShotIR-like specifications define per-shot props, characters, camera movement, shot type, and action order. When a generated video violates such specifications, however, users still lack a systematic interface for locating the failure and deciding the next intervention.

Current video-generation evaluators such as VBench~\cite{vbench2024}, EvalCrafter~\cite{evalcrafter2024}, VideoScore~\cite{videoscore2024}, and T2V-CompBench~\cite{t2vcompbench2024} report aggregate quality, motion, or alignment scores. These scores are useful for model comparison but weak as debugging signals: a low score does not indicate whether the character identity drifted, a required prop disappeared, camera motion was violated, or a specific temporal segment should be regenerated.
...
\]
where $c_i$ is a failure code, $(t_0^i,t_1^i)$ is the evidence span, $y_i$ is the affected target, $\sigma_i$ is confidence, $K_i$ contains representative keyframes, $T_i$ stores optional track-level evidence, and $a_i$ is a compiled repair action. The interface is designed to be machine-readable, human-auditable, and compatible with downstream video repair tools.

\paragraph{Scope.}
We separate the interface claim from the open-world generality claim. The expanded controlled fixture supports the claim that code--span--target--plan records provide auditable diagnosis and useful repair plans under known perturbations. A real-failure subset suggests transfer beyond deterministic perturbations, but broader coverage across generators, domains, styles, and long-form videos remains future work. We therefore call VideoGenDoctor-Bench-v0 a controlled diagnostic fixture rather than a general open-world benchmark.

\paragraph{Contributions.}
This paper contributes: (1) a code--span--target--plan representation for controllable video debugging; (2) a modular framework combining feature-based candidate generation, optional VLM verification, evidence aggregation, and patch compilation; (3) VideoGenDoctor-Bench-v0, with 324 controlled videos, 2,016 verified evidence spans, 9 perturbation types, and 12 observed codes, plus a 240-video real-failure subset covering 18 observed codes; and (4) an evaluation against direct VLM diagnosis, VLM-to-patch repair, blind human repair validation, per-code reliability analysis, paired bootstrap comparisons, and real-failure transfer checks.

\section{Related Work}
\label{sec:related}

\paragraph{Video generation evaluation.}
Existing benchmarks and evaluators measure aggregate video quality, motion smoothness, subject consistency, and text-video alignment~\cite{vbench2024,evalcrafter2024,videoscore2024,t2vcompbench2024}. They are valuable for ranking models but do not usually produce localized evidence or executable repair plans. VideoGenDoctor instead returns structured failure records tied to temporal spans and repair templates. A compact capability comparison is provided in Appendix~\ref{app:moved_main_details}, Table~\ref{tab:comparison}.

\paragraph{VLM critics and temporal grounding.}
Large vision-language models can inspect sampled frames and produce critiques~\cite{llava2023,internvl2024,gpt4v2023}. We therefore treat direct VLM reporting and VLM-to-patch generation as strong baselines rather than strawmen. The central question is whether constraining critique through code--span--target records improves temporal grounding, schema validity, auditability, repair executability, and cost. For temporal localization, we adapt temporal intersection-over-union:
\[
...
Program repair systems typically localize a defect, classify it, and generate a patch. VideoGenDoctor adapts this pattern to controllable video generation: each diagnosis must expose evidence and map to a supported repair family, connecting evaluation directly to iterative generation.
...
\end{overpic}
\caption{VideoGenDoctor framework overview. The system converts a generated video and its prompt or structured specification into failure-as-code diagnosis records, grounds each record in localized temporal evidence, maps records to structured repair plans, and verifies regenerated videos in a closed loop.}
\label{fig:system_overview}
\end{figure}

VideoGenDoctor contains four modules: segmentation and feature extraction, candidate diagnosis, optional VLM verification, and patch compilation. The first module samples keyframes and computes lightweight signals; the second applies thresholded rules to produce failure candidates; the third calibrates top-ranked candidates using structured VLM questions; and the fourth maps retained diagnosis records to structured repair actions such as reference injection, prop injection, camera override, segment regeneration, or style correction.

\subsection{Failure Records and Patch Templates}
VideoGenDoctor uses a machine-readable taxonomy with \TaxonomyTotalCodes{} codes grouped into Identity, Camera, Motion, Alignment, Style, and Scene. The current controlled fixture evaluates \TaxonomyNumCodes{} observed codes; the remaining codes define an extensible namespace and are not treated as empirically validated. The full taxonomy and group-level description are provided in Appendix~\ref{app:moved_main_details} and Appendix~\ref{app:taxonomy}.

Each predicted failure is represented as
\[
r=(c,t_0,t_1,y,\tilde{\sigma},K,T,a),
\]
where $c$ is the taxonomy code, $(t_0,t_1)$ is the evidence span, $y$ is the affected target, $\tilde{\sigma}$ is confidence, $K$ stores representative keyframes, $T$ stores optional tracks, and $a$ is the compiled repair action. Detectors and judges can be replaced without changing this schema.

\begin{table}[H]
\centering
\footnotesize
\caption{Representative failure types and repair-plan template mappings. A diagnosis record combines a failure type with a localized span, target, confidence, evidence, and repair-plan action.}
...
Given video $V$, we partition it into 2-second fixed-stride segments and sample six keyframes per segment. The default implementation computes semantic embedding drift with OpenCLIP~\cite{openclip2023}, optical flow with RAFT~\cite{raft2020} or OpenCV Farneback~\cite{opencv2000,farneback2003}, face drift with InsightFace~\cite{arcface2019}, and object or prop cues with YOLOv8~\cite{yolov8} when enabled. For each segment $s$ and failure code $c$, rule $\rho_c$ produces a score $\sigma_{s,c}\in[0,1]$; top-ranked candidates inherit segment boundaries and keyframes ranked by frame-level anomaly scores.
...
Each retained record stores an evidence object $E=(t_0,t_1,K,T)$ so that the diagnosis can be audited. Fixed segments are sufficient for the current fixture; future versions can refine spans using frame-level peaks and track-based merging. The patch compiler then emits either ShotIR field-level diffs or generator-agnostic repair plans. Each plan preserves the triggering code, evidence span, affected target, and rationale. In closed-loop evaluation, a video passes when all failure confidences fall below $\tau=0.5$ or the loop reaches $K_{\max}=3$; because this automatic criterion is verifier-dependent, Section~\ref{sec:repair_results} also reports blind human repair validation.
...
\label{sec:dataset}
We evaluate VideoGenDoctor as a diagnosis-and-repair system. The main questions are whether predicted failure codes are correct, evidence spans are temporally localized, repair plans are valid and useful, and repaired videos satisfy the specification under independent human judgment.

\paragraph{Controlled fixture.}
VideoGenDoctor-Bench-v0 contains 324 perturbed videos, 2,016 verified evidence spans, 9 deterministic perturbation types, and 12 observed failure codes. It is built from 18 clean source clips across six domain groups; each clip is paired with a ShotIR-style specification and perturbed by nine deterministic operators under two seeds. The fixture validates interface consistency, evidence auditability, and repair-planning usefulness under controlled failures; it is not intended as a comprehensive open-world benchmark. Construction details and operator-to-template alignment are moved to Appendix~\ref{app:moved_main_details}, Table~\ref{tab:controlled_fixture_composition}, and Appendix~\ref{app:additional_experiment_tables}, Table~\ref{tab:operator_alignment}.

\paragraph{Real-failure and real-normal subsets.}
We collect 240 naturally occurring failure videos from CogVideoX, Wan, Stable Video Diffusion, and HunyuanVideo~\cite{cogvideox2024,wan2025,svd2023,hunyuanvideo2025}. The subset is failure-enriched: from 480 raw generations, we retain videos only when annotators verify at least one visible specification violation. We also retain 120 real-normal videos from the same raw generations to estimate false positives and unnecessary repair behavior. Ambiguous and low-quality exclusions are treated as stress analyses rather than standard accuracy benchmarks. Detailed source composition, normal-set behavior, temporal evidence profiles, and stress tables are reported in Appendix~\ref{app:additional_experiment_tables}.

\paragraph{Annotation.}
A reliability subset of 120 videos and 768 candidate evidence spans is dual-annotated before adjudication. We report code-level Cohen's $\kappa$ and span-level annotator-to-annotator tIoU in Appendix~\ref{app:additional_experiment_tables}, Table~\ref{tab:annotation_reliability}. Agreement is substantial for failure codes and lower for exact temporal boundaries, reflecting the ambiguity of fine-grained evidence spans.

\begin{table}[H]
\centering\small
\caption{Dataset statistics. The first row is the expanded controlled perturbation fixture. The second row is the expanded real-failure subset collected from naturally occurring failures across multiple generators.}
...
We compare internal diagnosis variants (Prior-only, Rule-only, Rule+DummyJudge, Rule+Open-VLM, Rule+GPT-4V, and VideoGenDoctor-diagnosis), repair variants (VideoGenDoctor-patch, Patch+Judge, and VideoGenDoctor-full), and external VLM baselines including free-form VLM reporting, structured VLM reporting, self-consistency, VLM temporal proposal + patch, VLM-to-patch, and Score+LLM-to-patch. Most direct VLM baselines share the same hosted GPT-4V endpoint as Rule+GPT-4V to isolate prompt structure, output schema, and repair-planning protocol rather than model backbone. Whenever a table reports human repair success for a diagnosis-oriented front end, that front end is paired with the same downstream repair loop so that only the diagnostic interface changes. Implementation details are reported in Appendix~\ref{app:additional_experiment_tables}, Table~\ref{tab:vlm_details}; strengthened VLM-agent results are summarized in Appendix~\ref{app:moved_main_details}, Table~\ref{tab:strong_vlm_agent_baselines}.

We report macro-F1 and micro-F1 for failure-code detection, tIoU@0.3/0.5 and Top-$K$ keyframe hit rate for localization, automatic and human Pass@1/2 for repair, patch usefulness, new-artifact rate, and video-level bootstrap 95\% confidence intervals. All span-aware metrics use the same matching protocol: records are grouped by failure code, target-incompatible predictions are filtered, and remaining pairs are matched by Hungarian assignment~\cite{kuhn1955hungarian} using tIoU as the primary score and confidence only as a tie-breaker. Invalid or missing spans receive no localization credit.

\subsection{Failure-Code Detection}
\label{sec:f1_results}
\begin{table}[H]
\centering\small
\caption{Failure-code detection on the expanded controlled fixture. Direct VLM baselines are included to compare VideoGenDoctor with strong agent-style alternatives. Confidence intervals are estimated by video-level bootstrap resampling. Best values are boldfaced.}
...
VideoGenDoctor-diagnosis outperforms free-form and structured direct VLM reporting on macro-F1, while Rule+GPT-4V remains the strongest configuration. The larger gains in the localization results below indicate that the main advantage of structured diagnosis is not only detecting that a failure exists, but grounding it stably enough to support repair.

\subsection{Per-Code Reliability and Evidence Localization}
\label{sec:per_code}
\label{sec:evidence_results}
The controlled fixture covers twelve observed codes. Detailed per-code precision, recall, F1, and tIoU are reported in Appendix~\ref{app:additional_experiment_tables}, Table~\ref{tab:per_code_controlled}; the confusion matrix below summarizes dominant error modes.

\begin{figure}[H]
\centering
\safeincludegraphics[width=0.72\textwidth]{../figures/confusion_matrix.pdf}
\caption{Per-code confusion matrix for the 12 observed taxonomy codes on the expanded controlled fixture. The strongest off-diagonal confusions occur among camera and motion failures, while identity, prop-missing, compression-artifact, and scene-background failures remain comparatively concentrated on the diagonal.}
\label{fig:confusion_matrix}
\end{figure}

\begin{table}[H]
\centering\small
\caption{Evidence localization against human evidence spans on the expanded controlled fixture. Confidence intervals are estimated by video-level bootstrap resampling. Best values are boldfaced.}
...
Closed-loop repair is evaluated on the controlled fixture. Automatic Pass@1/2 is computed on all 324 videos; blind human Pass@1/2 is computed on a stratified 144-video subset. Human raters see the specification and output video but not method names or diagnosis records.

\begin{table}[t]
\centering\scriptsize
\caption{Closed-loop repair evaluation on the expanded controlled fixture. Automatic metrics are computed on all 324 controlled videos. Human metrics are computed on a stratified blind-evaluation subset of 144 videos. Best pass rates and patch-usefulness values are boldfaced; the new-artifact rate is reported for context rather than bold-ranked because abstention-heavy baselines are not directly comparable.}
...
The main repair gain is most consistent with structured repair planning and verification rather than with the final loop wrapper alone. The paired comparison in Section~\ref{sec:ablations} shows that the difference between Patch+Judge and VideoGenDoctor-full is small and not statistically decisive, so the full loop is best interpreted as traceable workflow integration.

\subsection{Controlled Fixture versus Real Generator Failures}
\label{sec:real_failures}
\begin{table}[H]
\centering\small
\caption{Generalization check on expanded controlled perturbations and naturally occurring generator failures. Macro-F1 and tIoU@0.5 are computed from each method's diagnosis front end; Patch useful and Human Pass@2 evaluate the corresponding downstream repair protocol. Best values within each subset are boldfaced.}
...
All methods degrade on naturally occurring failures, confirming that the controlled fixture is not a substitute for open-world evaluation. The real-failure subset contains more overlapping artifacts, less isolated evidence, and weaker alignment between specification fields and visible deviations. Per-code and per-generator results are reported in Appendix~\ref{app:additional_experiment_tables}, Tables~\ref{tab:real_per_code} and~\ref{tab:per_generator_results}.

\subsection{Ablation and Generalization Checks}
\label{sec:ablations}
\begin{table}[H]
\centering\scriptsize
\caption{Repair ablation on the expanded controlled fixture. We isolate the contribution of failure codes, temporal spans, evidence keyframes, patch templates, and verification. Best utility values are boldfaced; the new-artifact rate is reported for context rather than bold-ranked.}
...
The ablation tests whether localized evidence and template-constrained repair contribute beyond code prediction alone. The improvement from code-only to code+span and code+span+evidence settings supports the claim that evidence localization is operationally useful for repair planning.

\begin{table}[H]
\centering\scriptsize
\caption{Paired comparison of repair variants. We report paired bootstrap differences in blind human Pass@2. Small differences between Patch+Judge and Full loop should be interpreted cautiously.}
...
We also perform leave-one-perturbation-out and leave-one-source-group-out evaluations. Performance drops under held-out perturbations, especially for missing-event, camera-shake, and temporal-discontinuity operators, but remains above prior-only and score-only baselines. These results suggest that the system is not merely memorizing perturbation templates, while also confirming that unseen-operator generalization is weaker than in-distribution controlled evaluation. Detailed tables and the cost-performance trade-off are reported in Appendix~\ref{app:additional_experiment_tables}, Tables~\ref{tab:leave_one_perturbation}, \ref{tab:leave_one_source_group}, and Appendix~\ref{app:moved_main_details}, Table~\ref{tab:cost_performance}.

\section{Discussion}
\label{sec:discussion}

The current evidence supports a scoped claim: structured code--span--target--plan records improve diagnosis and repair planning over score-only feedback and direct VLM reporting on the controlled fixture and the current real-failure subset. It does not show that VideoGenDoctor solves open-world video repair. The controlled fixture demonstrates auditable localization under known perturbations, while the real-failure subset provides an initial but still limited transfer check.

The value of VideoGenDoctor should be assessed as an interface contribution rather than as a new video generation model or a learned repair algorithm. Its advantages lie in schema validity, temporal grounding, repair-plan usefulness, human auditability, and cost-performance. The modular design also allows feature extractors, rules, VLM judges, and patch compilers to be replaced independently; Stage-2 judging is restricted to top-ranked candidate segments, and the closed loop bounds repair by $K_{\max}$.

\paragraph{Limitations.}
The expanded fixture remains controlled; the real-failure subset is failure-enriched and cannot estimate natural failure prevalence; automatic pass rates are partly verifier-dependent; only observed taxonomy codes support empirical claims; and repair assumes access to a generator or editor that can consume patch actions. Broader validation requires larger generator coverage, longer videos, more domains and styles, stronger adapter executability tests, and larger multi-annotator studies.

\section{Conclusion}
We presented VideoGenDoctor, a structured framework for failure diagnosis, evidence localization, and repair planning in controllable video generation. The core contribution is an auditable code--span--target--plan interface that connects evaluation to actionable repair planning. The expanded evaluation consists of a 324-video controlled diagnostic fixture with 2,016 verified evidence spans and a 240-video real-failure subset with 1,536 verified evidence spans. The controlled fixture covers 12 observed taxonomy codes under 9 deterministic perturbation operators, while the real-failure subset covers 18 observed taxonomy codes across four publicly identifiable video generation systems. The primary empirical gain comes from structured repair planning and independent verification, while the full closed loop mainly provides traceable integration from diagnosis to evidence, patch planning, re-rendering, and verification. The broader generality claim remains scoped by the controlled and failure-enriched nature of the current evaluation sets.
...
\label{sec:repro}
The release package should include the reference implementation, report schema, taxonomy, patch compiler, controlled fixture generation scripts, annotations, prediction logs, evaluation scripts, prompt templates for VLM baselines, and scripts for bootstrap confidence intervals. Each run should record configuration files, model versions, package versions, random seeds, prompts, raw VLM responses, and output paths. Recomputing tables from serialized predictions and annotations should be CPU-only and reproducible in seconds; full feature extraction and VLM judging depend on the chosen configuration.
...
\begin{table}[H]
\centering\small
\caption{Capability comparison with related evaluation and critique paradigms. Direct VLM reporting is treated as a strong baseline rather than only comparing against score-only benchmarks.}
\label{tab:comparison}
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lccccc}
\toprule
\textbf{System} & \textbf{Score} & \textbf{Code} & \textbf{Evidence} & \textbf{Patch} & \textbf{Closed-Loop} \\
\midrule
...
\begin{table}[H]
\centering\scriptsize
\caption{Composition of the expanded controlled diagnostic fixture. Each domain group contains three clean source clips. Every source clip is paired with a ShotIR-style specification and perturbed by nine deterministic operators under two random seeds.}
...
\caption{Temporal distribution of verified evidence spans in the expanded evaluation sets. Panel (a) aggregates the 2,016 controlled-fixture spans over 2-second bins and shows the front- and mid-loaded pattern expected from deterministic perturbation operators. Panel (b) summarizes the 1,536 real-failure spans over the same time bins, making the later and more diffuse temporal mass directly comparable to panel (a). Panel (c) further breaks the real-failure spans down by failure group and time bin, highlighting that motion and camera failures dominate the later intervals and overlap more strongly than in the controlled fixture.}
\label{fig:temporal_evidence_distribution}
\end{figure}

\begin{table}[H]
\centering\scriptsize
\caption{Strengthened VLM-agent baselines on the expanded controlled fixture. All rows are evaluated under the same downstream repair protocol; diagnosis-only front ends are explicitly paired with that shared repair loop when human repair success is reported. Self-consistency improves direct VLM reporting but increases cost. The validity column reports whether the emitted action stays inside the supported repair-plan vocabulary; it does not claim generator-adapter executability. Best values are boldfaced; higher is better except for relative cost, where lower is better.}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.82\textwidth]{../figures/manuscript_fig3.pdf}
\caption{Closed-loop repair comparison on the expanded controlled fixture. Left: automatic verifier Pass@1/2 on all 324 controlled videos. Right: blind human Pass@1/2 on the stratified 144-video human-evaluation subset. Automatic pass rates may be coupled to the system verifier, while human pass rates provide an independent check of repair success.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Operator-template alignment analysis for the expanded controlled fixture. The table makes explicit which observable cues, rule signals, failure codes, and repair-plan templates are aligned by construction.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Source composition of the expanded real-failure subset. The subset is failure-enriched: we generate 480 raw videos and retain 240 samples with annotator-verified visible failures. It is used only as an initial transfer check, not to estimate natural failure prevalence. We use publicly identifiable generator checkpoints to make the subset construction auditable and reproducible.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Real-failure and real-normal evaluation. The real-normal subset contains 120 videos retained from the same 480 raw generations but judged to contain no obvious failure. Best values are boldfaced; higher is better except for real-normal FPR and unnecessary patch rate, where lower is better.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Stress analysis on 72 ambiguous specification-violation cases. These samples contain possible but non-adjudicable deviations from the specification. Best values are boldfaced; higher is better for abstention-oriented columns, lower is better for false concrete patch, wrong-target patch, over-repair, and new artifacts.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Stress analysis on 48 low-quality or non-adjudicable cases. These samples are dominated by global degradation, unclear content, or insufficient evidence for reliable code-level labels. Best values are boldfaced; higher is better for quality-gated abstention, lower is better for the remaining columns.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Observed failure-code distribution in the expanded controlled fixture and real-failure subset. The full taxonomy contains 38 codes, but empirical claims are restricted to the observed codes below.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Per-code reliability of the default VideoGenDoctor diagnosis configuration on the expanded controlled fixture.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Per-code reliability on the expanded real-failure subset. The subset is more challenging than the controlled fixture, especially for motion, camera, and scene-level failures.}
...
\tblcode{ID\_MULTIPLE\_FACES} & Multiple distinct faces appear when one character is expected. & Count distinct identities detected in a single-character shot. & \tblcode{reduce\_character\_count} \\
...
\tblcode{SC\_BG\_FLICKER} & The background flickers or pulses unnaturally. & Detect high-frequency luminance variation in background regions. & \tblcode{smooth\_background} \\
...
\tblcode{AL\_PROP\_OCCLUDED} & A required prop is present but substantially occluded. & Estimate visible area and occlusion of the prop box. & \tblcode{adjust\_prop\_placement} \\
\midrule
...
The expanded experiment fixture includes a dual-annotated reliability subset of 120 videos and

...
\end{overpic}
\caption{Overview of VideoGenDoctor. The system converts a generated video and its prompt or structured specification into failure-as-code records, grounds each record in localized temporal evidence, compiles a structured repair plan, and verifies regenerated videos in a bounded closed loop.}
\label{fig:system_overview}
\end{figure}

VideoGenDoctor contains four modules: segmentation and feature extraction, candidate diagnosis, optional VLM verification, and patch compilation. The first module samples keyframes and computes lightweight signals; the second applies thresholded rules to produce failure candidates; the third calibrates top-ranked candidates using structured VLM questions; and the fourth maps retained diagnosis records to structured repair actions such as reference injection, prop injection, camera override, segment regeneration, or style correction. We distinguish repair-plan validity from adapter executability: a plan is valid when it names a supported action, target, and evidence span, while it is executable only when the downstream generator or editor exposes the required control interface.
...
\centering
\footnotesize
\caption{Representative mappings from failure types to evidence signals and repair-plan templates. Each diagnosis record stores the failure code, localized span, affected target, confidence, evidence, and compiled repair action.}
...
\begin{table}[H]
\centering\small
\caption{Dataset statistics for the controlled fixture and failure-enriched real-generator subset. The real-failure split contains naturally occurring, annotator-verified specification violations from multiple generators and is not used to estimate failure prevalence.}
...
\begin{table}[H]
\centering\small
\caption{Failure-code detection on the controlled fixture. Direct VLM baselines test whether schema-constrained diagnosis improves over strong free-form and structured critique alternatives. Confidence intervals are estimated by video-level bootstrap resampling; best values are boldfaced.}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{../figures/confusion_matrix.pdf}
\caption{Per-code confusion matrix for the 12 observed taxonomy codes on the controlled fixture. The strongest off-diagonal confusions occur among camera and motion failures, while identity, prop-missing, compression-artifact, and scene-background failures remain comparatively concentrated on the diagonal.}
\label{fig:confusion_matrix}
\end{figure}

\begin{table}[H]
\centering\small
\caption{Evidence localization against human evidence spans on the controlled fixture. Confidence intervals are estimated by video-level bootstrap resampling; best values are boldfaced.}
...
\begin{table}[H]
\centering\small
\caption{Transfer from controlled perturbations to naturally occurring generator failures. Macro-F1 and tIoU@0.5 evaluate each diagnosis front end; patch usefulness and Human Pass@2 evaluate the corresponding downstream repair protocol. Best values within each subset are boldfaced.}
...
\begin{table}[H]
\centering\scriptsize
\caption{Repair ablation on the controlled fixture. The variants isolate the contributions of failure codes, affected targets, temporal spans, evidence keyframes, patch templates, and verification. Best utility values are boldfaced; the new-artifact rate is reported for context rather than bold-ranked.}
...
We further examine the effect of the diagnosis threshold used to retain failure records before repair planning. Table~\ref{tab:threshold_sensitivity} and Figure~\ref{fig:threshold_sensitivity} summarize a draft threshold sweep over $\tau \in \{0.30,0.40,0.50,0.60,0.70\}$. The default threshold $\tau=0.50$ provides the strongest balance in this sweep: macro-F1 and tIoU@0.5 peak near the default setting, while real-normal false positives and unnecessary patching decrease monotonically as the threshold becomes more conservative. The trend supports using an intermediate threshold rather than an aggressive low-threshold setting that over-triggers repair plans or a high-threshold setting that suppresses useful diagnoses.

\begin{table}[H]
\centering\scriptsize
\caption{Threshold sensitivity for retaining diagnosis records before repair planning. Lower thresholds increase repair coverage but also raise real-normal false positives and unnecessary patching; higher thresholds reduce repair risk at the cost of missed diagnoses.}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.92\textwidth]{../figures/threshold_sensitivity.pdf}
\caption{Threshold sensitivity of diagnosis and repair-risk metrics. The default threshold $\tau=0.50$ is marked with a vertical dashed line and gives the best diagnosis-localization trade-off, while more conservative thresholds reduce false positives and unnecessary patching.}
\label{fig:threshold_sensitivity}
\end{figure}

\paragraph{Stage-2 weight and candidate budget.}
We also sweep the Stage-2 judge weight $\alpha$ and the number of candidate segments passed to the judge. Figure~\ref{fig:alpha_topk_sensitivity} shows that $\alpha=0.6$ and top-$K=3$ provide the best operating point among the tested settings. Removing the Stage-2 contribution ($\alpha=0$) weakens both macro-F1 and Human Pass@2, while assigning too much weight to the judge ($\alpha=0.9$) reduces stability, consistent with occasional VLM over-correction. Increasing top-$K$ from 1 to 3 improves performance, but moving from 3 to 5 gives little additional benefit and slightly increases exposure to noisy candidates.

\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{../figures/alpha_topk_sensitivity.pdf}
\caption{Sensitivity to Stage-2 judge weight $\alpha$ and top-$K$ candidate segments. Each cell reports the draft metric value for a particular $(\alpha,K)$ pair. The default setting, $\alpha=0.6$ and $K=3$, yields the strongest balance between diagnosis accuracy and downstream human repair success.}
\label{fig:alpha_topk_sensitivity}
\end{figure}

\begin{table}[H]
\centering\scriptsize
\caption{Paired comparison of repair variants. Differences are paired bootstrap estimates in blind human Pass@2. Small differences between Patch+Judge and the full loop should be interpreted cautiously.}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{../figures/manuscript_fig3.pdf}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.92\textwidth]{../figures/cost_performance_pareto.pdf}
...
\subsection{Draft Extensions for Experiment Design}
\label{app:draft_experiment_extensions}

\paragraph{Adapter executability.}
Table~\ref{tab:adapter_executability_draft} and Figure~\ref{fig:adapter_executability} separate repair-plan quality from backend executability by grouping generated plans into L0--L3 adapter levels. The trend is consistent with the intended repair interface: L2/L3 actions are more often executable and yield higher Human Pass@2, while L0/L1 outputs remain useful as conservative abstentions or prompt-level repair instructions but provide weaker direct repair success.

\begin{table}[H]
\centering\scriptsize
\caption{Draft adapter-executability stratification. L0 denotes no-op or abstention; L1 denotes prompt-level repair instructions; L2 denotes partially executable repair actions; L3 denotes native generator/editor controls.}
\label{tab:adapter_executability_draft}
\begin{tabular}{lcccc}
\toprule
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{../figures/adapter_executability.pdf}
...
\begin{table}[H]
\centering\scriptsize
\caption{Draft stress validation on harder real-failure slices. Intervals denote bootstrap-style half-widths used for plotting uncertainty in Figure~\ref{fig:extended_real_validation}.}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.95\textwidth]{../figures/extended_real_validation.pdf}
\caption{Draft stress validation on harder real-failure slices. Left: diagnosis, localization, and human repair metrics with uncertainty bars. Right: false-positive rate on matched real-normal controls for each slice.}
\label{fig:extended_real_validation}
\end{figure}

\paragraph{Multi-annotator stability.}
Table~\ref{tab:multi_annotator_stability_draft} and Figure~\ref{fig:multi_annotator_stability} extend the annotation reliability analysis from dual annotation to a three-annotator draft setting. Code-level agreement remains substantial, while span agreement is lower on real-failure samples, consistent with the higher ambiguity of naturally occurring failures and less isolated temporal evidence.

\begin{table}[H]
\centering\scriptsize
\caption{Draft three-annotator stability analysis. Boundary disagreement is measured in seconds; lower is better.}
\label{tab:multi_annotator_stability_draft}
\begin{tabular}{lccccc}
\toprule
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.90\textwidth]{../figures/multi_annotator_stability.pdf}
\caption{Draft multi-annotator stability analysis. Code-level Fleiss' $\kappa$ is higher than temporal-boundary agreement, and the real-failure subset remains the most ambiguous split.}

...
\centering
\footnotesize
\caption{Failure types, the signals that detect them, and the repair actions they trigger.}
...
A 120-video reliability subset with 768 candidate evidence spans is dual-annotated before adjudication. Code-level agreement reaches Cohen's $\kappa = \IAAcode$; span-level agreement is lower (tIoU = \IAAspan), reflecting inherent ambiguity in fine-grained temporal boundaries (Appendix~\ref{app:additional_experiment_tables}, Table~\ref{tab:annotation_reliability}).

\begin{table}[H]
\centering\small
\caption{Dataset statistics. The real-failure split is failure-enriched and cannot estimate natural prevalence.}
...
The controlled fixture covers twelve observed codes. Per-code F1 ranges from 0.861 (event-missing) to 0.958 (compression-artifact), with temporal localization following a similar pattern: frozen-frame and compression-artifact detection are most reliable (tIoU@0.5 above 0.77), while event-missing and camera-shake are hardest to localize precisely (tIoU@0.5 below 0.70). Full per-code precision, recall, F1, and tIoU are in Appendix~\ref{app:additional_experiment_tables}, Table~\ref{tab:per_code_controlled}. The confusion matrix (Figure~\ref{fig:confusion_matrix}) confirms that camera and motion codes account for the majority of off-diagonal errors.
...
The main repair gain comes from structured diagnosis and verification, not from the loop wrapper alone. Random-patch and re-render-all baselines confirm that undirected repair is substantially worse than diagnosis-guided patching: re-render-all achieves Pass@2 of 0.543 (vs.\ 0.764 for VideoGenDoctor-full) at higher regeneration cost. The paired comparison in Section~\ref{sec:ablations} shows that the difference between Patch+Judge and VideoGenDoctor-full is small and not statistically decisive; the full loop is best interpreted as traceable workflow integration rather than a distinct algorithmic contribution.

\subsection{Controlled Fixture versus Real Generator Failures}
\label{sec:real_failures}
Figure~\ref{fig:repair_curve} in Appendix~\ref{app:moved_main_details} visualizes closed-loop repair as bar charts; Figure~\ref{fig:controlled_vs_real_paired} below shows the controlled-to-real transfer across all four metrics in a paired bar-chart format.
\begin{table}[H]
\centering\small
...
\caption{Controlled-to-real transfer across four metrics. Solid bars: controlled fixture; hatched bars: real-failure subset. The degradation gap (annotated) is largest for tIoU@0.5 and smallest for patch usefulness.}
...
Table~\ref{tab:feature_ablation} ablates each feature signal from the default VideoGenDoctor-diagnosis configuration. Optical flow is the single most critical signal: removing it drops macro-F1 from \FeatFullMacroF{} to \FeatNoFlowMacroF{} and tIoU@0.5 from \FeatFullTIoU{} to \FeatNoFlowTIoU. CLIP embedding drift is next, particularly for scene and style codes. Face embedding removal mainly affects identity-code precision. The object detection signal has the smallest individual impact because YOLOv8 is only active for prop-related codes. Single-signal configurations (CLIP-only, flow-only, face-only) all perform substantially worse, confirming that the signals complement each other.

\begin{table}[H]
\centering\scriptsize
\caption{Per-feature ablation of the default diagnosis configuration. Each row removes one signal or uses a single signal; Human Pass@2 is measured after pairing the ablated diagnosis front end with the shared repair loop.}
...
\caption{Per-feature ablation. Optical flow is the single most critical signal; single-signal configurations (bottom three rows) all underperform substantially, confirming complementary contributions.}
...
Each record component (code, target, span, evidence keyframes, patch template, and verification) adds measurable repair value beyond code prediction alone. The jump from code-only (Pass@2 0.458) to code+span (0.625) and code+span+evidence (0.688) shows that temporal evidence is the largest single contributor after the code itself.

\begin{figure}[H]
\centering
\safeincludegraphics[width=\textwidth]{../figures/repair_ablation_bars.pdf}
\caption{Repair ablation as grouped bars. Human Pass@2 (dark blue) and patch usefulness (light blue, scaled) rise additively as code, span, evidence, and verification are incorporated. New-artifact rates (annotated at bottom) trend downward as diagnosis quality improves.}
\label{fig:repair_ablation_bars}
\end{figure}

\paragraph{Threshold sensitivity.}
Table~\ref{tab:threshold_sensitivity} and Figure~\ref{fig:threshold_sensitivity} sweep the diagnosis retention threshold $\tau \in \{0.30,0.40,0.50,0.60,0.70\}$. Macro-F1 and tIoU@0.5 peak at $\tau=0.50$; false positives and unnecessary patching decrease monotonically with larger $\tau$. Aggressive low thresholds over-trigger repairs; conservative high thresholds suppress useful diagnoses.

\begin{table}[H]
\centering\scriptsize
\caption{Threshold sensitivity. $\tau=0.50$ balances diagnosis coverage against false positives and unnecessary patching on real-normal videos.}
...
\caption{Threshold sensitivity curves. Dashed line at $\tau=0.50$ marks the operating point where macro-F1 and tIoU@0.5 peak before false positives rise sharply.}
\label{fig:threshold_sensitivity}
\end{figure}

\paragraph{Stage-2 weight and candidate budget.}
Figure~\ref{fig:alpha_topk_sensitivity} sweeps the judge weight $\alpha$ and candidate budget $K$. $\alpha=0.6$ with $K=3$ gives the best operating point. Removing Stage-2 ($\alpha=0$) reduces both diagnosis and repair metrics; $\alpha=0.9$ introduces instability from VLM over-correction. Increasing $K$ from 1 to 3 helps, but 3 to 5 adds little while exposing the judge to more noisy candidates.

\begin{figure}[H]
\centering
\safeincludegraphics[width=\textwidth]{../figures/alpha_topk_sensitivity.pdf}
\caption{Sensitivity to judge weight $\alpha$ and candidate budget $K$. $\alpha=0.6$, $K=3$ balances diagnosis accuracy and repair success; $\alpha=0.9$ introduces instability from VLM over-correction.}
\label{fig:alpha_topk_sensitivity}
\end{figure}

\begin{table}[H]
\centering\scriptsize
\caption{Paired bootstrap differences in blind human Pass@2. Verification adds a reliable gain (+0.111); the full-loop wrapper adds a small, non-decisive increment (+0.021).}
...
Leave-one-perturbation-out and leave-one-source-group-out evaluations confirm that performance drops under held-out operators---particularly missing-event, camera-shake, and temporal-discontinuity perturbations---but remains above prior-only and score-only baselines (Appendix~\ref{app:additional_experiment_tables}, Tables~\ref{tab:leave_one_perturbation}--\ref{tab:leave_one_source_group}). The system does not simply memorize perturbation templates, though unseen-operator generalization is weaker than in-distribution evaluation. Additional checks cover adapter executability, real-failure stress slices, and multi-annotator stability (Tables~\ref{tab:adapter_executability}--\ref{tab:multi_annotator_stability}).

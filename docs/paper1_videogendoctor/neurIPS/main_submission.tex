...
\title{VideoGenDoctor: Structured Diagnosis, Evidence Localization, and Repair\\for Controllable Video Generation}
...
A 120-video reliability subset with 768 candidate evidence spans is dual-annotated before adjudication. Code-level agreement reaches Cohen's $\kappa = \IAAcode$; span-level agreement is lower (tIoU = \IAAspan), reflecting inherent ambiguity in fine-grained temporal boundaries. Dataset statistics are in Appendix~\ref{app:moved_main_details} (Table~\ref{tab:dataset_statistics}); annotation details are in Appendix~\ref{app:additional_experiment_tables} (Table~\ref{tab:annotation_reliability}).

\subsection{Compared methods and metrics}
We compare three groups of methods. \textbf{Diagnosis variants:} Prior-only (a frequency-based prior that predicts the most common failure codes per perturbation type without inspecting the video), Rule-only, Rule+DummyJudge, Rule+Open-VLM, Rule+GPT-4V, and VideoGenDoctor-diagnosis. \textbf{Repair variants:} VideoGenDoctor-patch (applies template-based patches directly from diagnosis output without verification), Patch+Judge, and VideoGenDoctor-full. \textbf{External VLM baselines:} free-form VLM reporting, structured VLM reporting, self-consistency, temporal proposal + patch, VLM-to-patch, and Score+LLM-to-patch. Direct VLM baselines share the same GPT-4V endpoint as Rule+GPT-4V to isolate prompt structure, output schema, and repair protocol from model backbone; Rule+Open-VLM provides an open-model reference. Diagnosis-only front ends are paired with the same downstream repair loop whenever human repair success is reported. Implementation details are in Appendix~\ref{app:additional_experiment_tables} (Table~\ref{tab:vlm_details}); strengthened VLM-agent baselines are in Appendix~\ref{app:moved_main_details} (Table~\ref{tab:strong_vlm_agent_baselines}).
...
Random-patch and re-render-all baselines demonstrate that undirected repair is substantially weaker than diagnosis-guided patching: re-render-all achieves Pass@2 of 0.543 (vs.\ 0.764 for VideoGenDoctor-full) at higher regeneration cost. The paired comparison in Section~\ref{sec:ablations} shows a small and non-decisive difference between Patch+Judge and VideoGenDoctor-full. Because the automatic stopping criterion depends on the system verifier, we measure agreement between the automatic verifier and blind human judgment on the 144-video subset; agreement reaches \VerifierHumanAgreement{} (Cohen's $\kappa = \VerifierHumanKappa$). The primary Pass@K claims are therefore based on blind human judgment, with automatic Pass@K serving as a scalable proxy for ablation sweeps.
...
We further examine sensitivity to three hyperparameters. First, Table~\ref{tab:threshold_sensitivity} sweeps the diagnosis retention threshold over $\tau \in \{0.30,\dots,0.70\}$. Macro-F1 and tIoU@0.5 peak at $\tau=0.50$; lower thresholds increase real-normal false positives from 0.100 to 0.158, while higher thresholds suppress useful diagnoses (macro-F1 drops to 0.886 at $\tau=0.70$). Figure~\ref{fig:threshold_sensitivity} in Appendix~\ref{app:additional_experiment_tables} visualizes the full sweep curves.

\begin{table}[t]
\centering\scriptsize
\caption{Threshold sensitivity for retaining diagnosis records. $\tau=0.50$ balances coverage against false positives.}
...
Second, the Stage-2 judge weight $\alpha=0.6$ and candidate budget $K=3$ jointly give the best operating point. Removing Stage-2 ($\alpha=0$) reduces both diagnosis and repair metrics, while $\alpha=0.9$ introduces instability from VLM over-correction (Appendix~\ref{app:additional_experiment_tables}, Figure~\ref{fig:alpha_topk_sensitivity}). Third, Table~\ref{tab:paired_repair_test} reports paired bootstrap comparisons: verification adds a reliable gain (+0.111 in Human Pass@2, 95\% CI [0.021, 0.215]), while the full-loop wrapper contributes a small, non-decisive increment (+0.021, 95\% CI [$-$0.063, 0.104]).

\begin{table}[t]
\centering\scriptsize
\caption{Paired bootstrap differences in blind human Pass@2.}
...
\begin{ack}
This work was supported by the authors' respective institutions. We thank the anonymous reviewers for their constructive feedback.
...
\paragraph{Taxonomy and patch mappings.}
Table~\ref{tab:code_patch_mapping} lists the representative failure types covered by the VideoGenDoctor taxonomy, the evidence signals used to detect each failure, and the repair action the patch compiler emits. These seven categories map to the six taxonomy groups defined in Table~\ref{tab:failure_taxonomy_groups} and serve as the primary diagnosis-to-repair bridge.
...
\paragraph{Controlled fixture composition.}
Table~\ref{tab:controlled_fixture_composition} breaks down the controlled diagnostic fixture by domain group. Each of the six groups contributes three clean source clips, which are then perturbed by nine deterministic operators under two random seeds. The table records per-group clip counts, average durations, resolutions, perturbation counts, seeds, and resulting video counts. This construction ensures that every taxonomy code in the controlled evaluation is backed by a known perturbation operator-template alignment (see Appendix~\ref{app:additional_experiment_tables}, Table~\ref{tab:operator_alignment}).
...
\paragraph{Strengthened VLM-agent baselines.}
Table~\ref{tab:strong_vlm_agent_baselines} reports strengthened VLM-agent baselines on the controlled fixture, all evaluated under the same downstream repair protocol so that diagnosis-only front ends are fairly compared. Free-form VLM reporting achieves the lowest schema validity and patch vocabulary validity among structured methods. Adding a structured JSON schema improves schema validity to 0.914 and macro-F1 to 0.887. Self-consistency (3-run majority voting) further improves macro-F1 to 0.898 at 3.48$\times$ the cost. Temporal proposal with patch output achieves a favorable tIoU@0.5 of 0.702 by asking the VLM to propose spans explicitly. VideoGenDoctor-full exceeds all direct VLM baselines on schema validity (0.991) and patch vocabulary validity (0.953) while being the most cost-efficient structured configuration. Rule+GPT-4V provides the strongest absolute scores at 2.75$\times$ the cost of VideoGenDoctor-full.
...
Table~\ref{tab:cost_performance} compares cost, latency, and performance across five configurations under a shared downstream repair protocol. Rule-only achieves the lowest cost (0.42$\times$) but leaves a sizable gap in human Pass@2 (0.599 vs.\ 0.764 for VideoGenDoctor-full). Rule+Open-VLM narrows the gap to Pass@2 0.716 at 0.73$\times$ the cost. Rule+GPT-4V achieves the strongest Human Pass@2 (0.792) at 2.75$\times$ the cost. Direct VLM structured reporting, which bypasses the rule-based candidate stage, reaches Pass@2 0.681 at 1.16$\times$ the cost, underperforming all rule-assisted configurations. VideoGenDoctor-full represents the recommended default, balancing strong repair performance with moderate cost. Figure~\ref{fig:cost_performance_pareto} visualizes this trade-off as a Pareto front, confirming that VideoGenDoctor-full occupies the knee point: beyond it, marginal Pass@2 gains come at steeply increasing relative cost.
...
\paragraph{Controlled-to-real transfer.}
Figure~\ref{fig:controlled_vs_real_paired} compares VideoGenDoctor and the structured VLM baseline on four core metrics across the controlled fixture (solid bars) and real-failure subset (hatched bars). The degradation gap is largest for tIoU@0.5 (--18.3\%), reflecting the diffuse, overlapping evidence patterns in uncurated videos, and smallest for patch usefulness (--7.1\%), indicating that repair-plan quality degrades more gracefully than localization accuracy.
...
Figure~\ref{fig:feature_ablation} visualizes the per-feature ablation results from Table~\ref{tab:feature_ablation} as a horizontal bar chart, ordered by contribution. Optical flow removal causes the largest drop in macro-F1 and tIoU@0.5, followed by CLIP embedding and face embedding. Single-signal configurations (CLIP only, flow only, face only) cluster at substantially lower performance levels, confirming that the signals carry complementary information and that no single signal can substitute for the full set.
...
\paragraph{Repair ablation.}
Figure~\ref{fig:repair_ablation_bars} shows the additive contribution of each diagnosis-record component to downstream repair success. Human Pass@2 rises from 0.278 (score-only) to 0.458 (code-only patch), 0.625 (code+span), 0.688 (code+span+evidence), and ultimately 0.764 (VideoGenDoctor-full). The jump from code-only to code+span is the largest single increment, underscoring the value of temporal localization beyond mere failure-code prediction.
...
Figure~\ref{fig:threshold_sensitivity} plots macro-F1, tIoU@0.5, and real-normal FPR as functions of the diagnosis retention threshold $\tau$. Macro-F1 and tIoU@0.5 peak at $\tau=0.50$ (dashed line), which balances coverage against false positives. Lower thresholds ($\tau=0.30$) inflate FPR to 0.158; higher thresholds ($\tau=0.70$) suppress useful diagnoses, dropping macro-F1 to 0.886.
...
\paragraph{Alpha and top-K sensitivity.}
Figure~\ref{fig:alpha_topk_sensitivity} examines the joint sensitivity to the Stage-2 judge weight $\alpha$ and the candidate budget $K$. The default $\alpha=0.6$, $K=3$ provides the best balance of diagnosis accuracy and repair success. Removing Stage-2 ($\alpha=0$) reduces both diagnosis and repair metrics, while $\alpha=0.9$ introduces instability from VLM over-correction on codes outside its effective visual range.
...
\paragraph{Operator-template alignment.}
Table~\ref{tab:operator_alignment} documents the alignment between each perturbation operator, its observable cue, the rule or feature signal that detects it, the failure code it triggers, and the repair-plan template it maps to. The nine controlled-fixture operators cover all six taxonomy groups. Each alignment type (identity-anchor, prop-presence, motion-control, framing-control, stability-control, event-structure, temporal-continuity, quality-restoration, scene-anchor) pairs with a specific repair action, ensuring that the controlled evaluation exercises the full diagnosis-to-repair bridge.
...
\paragraph{Real-failure source composition.}
Table~\ref{tab:real_failure_source} details the four production generators (CogVideoX, Wan, Stable Video Diffusion, HunyuanVideo) used to construct the real-failure subset. For each generator, the table records the version or checkpoint identifier, input type (text with ShotIR-to-prompt templates, image-to-video with reference frames, or multi-shot prompts), the number of prompts or specifications, the number of retained videos, average duration, and the count of unique taxonomy codes observed. Sampling is balanced at 60 retained failures per generator. Generator-specific adapters are logged in per-video JSON manifests (see Appendix~\ref{app:real_failure_manifest}).
...
\paragraph{Real-failure and real-normal evaluation.}
Table~\ref{tab:real_normal_control} reports recall on the 240 real-failure videos and false-positive rate (FPR) on the 120 real-normal controls. VideoGenDoctor-full achieves the lowest FPR (0.100) and unnecessary patch rate (0.087) while maintaining competitive recall (0.858). Score-only feedback achieves the highest FPR (0.258) and lowest specificity (0.742), confirming that scalar evaluators over-flag benign variations that rule-based screening and VLM verification correctly filter out. The unnecessary patch rate measures the fraction of real-normal videos where the pipeline generates at least one concrete (non-no-op) patch action.
...
\paragraph{Ambiguous specification-violation cases.}
Table~\ref{tab:ambiguous_violation_cases} reports stress-test results on 72 samples with possible but non-adjudicable specification violations. These boundary cases are the hardest evaluation regime: the correct behavior is often abstention rather than a concrete patch. VideoGenDoctor-diagnosis achieves the highest abstention rate (0.500) and appropriate abstention rate (0.639), while VLM structured reporting generates the highest false concrete patch rate (0.611). VideoGenDoctor-full achieves the lowest wrong-target patch rate (0.111) and over-repair rate (0.181), confirming that the pipeline's evidence thresholds and template constraints reduce harmful patching on ambiguous inputs.
...
\paragraph{Low-quality and non-adjudicable cases.}
Table~\ref{tab:low_quality_cases} evaluates the pipeline on 48 samples dominated by global degradation (e.g., heavy noise, extreme blur), where reliable code-level labeling is impossible even for human annotators. VideoGenDoctor-diagnosis achieves the highest quality-gated abstention rate (0.667), correctly refusing to emit concrete patches when visual evidence is insufficient. By contrast, VLM structured reporting generates false concrete patches on 0.458 of these samples and wrong-target patches on 0.188, indicating that without rule-based quality gating, VLMs are prone to over-confident diagnosis on degraded inputs. VideoGenDoctor-full achieves the lowest false concrete patch rate (0.125) and wrong-target patch rate (0.042).
...
\paragraph{Observed failure-code distribution.}
Table~\ref{tab:code_distribution} lists the 18 taxonomy codes observed across the controlled and real-failure evaluation sets, together with their span counts and group membership. Twelve codes appear in the controlled fixture; an additional six codes (e.g., clothing change, wrong camera angle, action-order error, text-prompt mismatch, color shift, object teleportation) appear only in the real-failure subset because they are not straightforward to produce deterministically. Empirical claims in the main text are restricted to these observed codes; the remaining 20 codes in the full 38-code taxonomy are definitional.
...
Table~\ref{tab:annotation_reliability} reports dual-annotation agreement on a 120-video reliability subset (768 candidate evidence spans). Code-level agreement reaches Cohen's $\kappa=0.831$ on the combined subset, with higher agreement on controlled samples ($\kappa=0.858$) than on real-failure samples ($\kappa=0.803$). Span-level agreement follows a similar pattern (tIoU = 0.716 controlled, 0.641 real-failure, 0.681 combined), reflecting the inherently greater temporal-boundary ambiguity in uncurated videos.
...
\paragraph{VLM implementation details.}
Table~\ref{tab:vlm_details} documents the implementation details for all VLM-based configurations: model endpoint, number of sampled frames, resolution, prompt type, and relative cost. Most direct VLM baselines share the same hosted GPT-4V endpoint to isolate prompt- and schema-level effects from backbone model changes; Rule+Open-VLM uses Qwen2-VL-7B-Instruct as an open-model reference. The self-consistency baseline runs 3 independent VLM calls and applies majority voting, improving macro-F1 by +0.011 over single-run structured reporting at 3.48$\times$ the cost. All hosted endpoint calls log the raw prompt, response, latency, and output path for auditability.
...
\midrule
Rule+Open-VLM & Qwen2-VL-7B-Instruct~\cite{qwen2vl2024} & 18 & 512px short side & structured QA & 0.34$\times$ \\
...
\paragraph{Per-code reliability on real failures.}
Table~\ref{tab:real_per_code} reports precision, recall, F1, and tIoU@0.5 for each of the 18 observed codes on the real-failure subset. Frozen-frame detection remains the strongest code (F1 0.876, tIoU@0.5 0.663), consistent with its unambiguous near-zero-flow signal. The hardest codes are action-order errors (F1 0.769, tIoU@0.5 0.524) and event-missing (F1 0.784, tIoU@0.5 0.542), where the VLM judge must reason about temporal order and event presence from sparse keyframes. Camera- and motion-group codes exhibit the largest drop from controlled to real, reflecting the greater diversity and overlap of real-world camera and motion artifacts.
...
\paragraph{Per-generator results.}
Table~\ref{tab:per_generator_results} breaks down real-failure performance by generator. Stable Video Diffusion achieves the strongest macro-F1 (0.842) and lowest FPR (0.083), likely because its reference-frame conditioning provides a stronger identity anchor than text-only prompts. HunyuanVideo is the most challenging generator (macro-F1 0.809, FPR 0.117), consistent with its multi-shot prompt input which introduces more opportunities for camera, motion, and alignment failures. The main failure groups per generator align with the input modality: SVD struggles most with identity/style/scene, while text-conditioned generators (CogVideoX, Wan, HunyuanVideo) are dominated by camera/motion/alignment failures.
...
\paragraph{Leave-one-perturbation-out.}
Table~\ref{tab:leave_one_perturbation} evaluates generalization by holding out each perturbation operator from threshold tuning and testing on it alone. Average macro-F1 drops to 0.870 (from 0.911 in the full configuration), confirming that the pipeline does not simply memorize perturbation templates, though unseen-operator generalization is weaker than in-distribution evaluation. The hardest held-out perturbations are action dropping/shifting (macro-F1 0.838) and camera-shake injection (0.851), while compression re-encoding (0.910) remains the easiest to detect even without calibration on that specific perturbation.
...
\paragraph{Leave-one-source-group-out.}
Table~\ref{tab:leave_one_source_group} evaluates robustness across the six domain groups by holding out each group from threshold tuning. Average macro-F1 of 0.891 and tIoU@0.5 of 0.704 indicate reasonable domain generalization, with style/lighting variation (macro-F1 0.907) and indoor/human-object (0.899) as the easiest groups, and camera-control scene (0.875) and scene-change/background (0.884) as the hardest. The performance spread across groups is narrower than the spread across perturbation types (Table~\ref{tab:leave_one_perturbation}), suggesting the pipeline is more sensitive to perturbation type than to scene domain.
...
\paragraph{Patch-to-generator adapter details.}
Table~\ref{tab:patch_adapter_details} documents the full adapter availability matrix: for each repair-plan action, it lists the corresponding ShotIR specification field, the adapter-executability level (L0--L3), the concrete generator or editor input that executes the action, and the fallback behavior when the adapter is unavailable. Reference-frame injection and segment regeneration are the most widely supported actions (L2 for image-to-video backends; L1 partial for text-to-video), while camera-angle and prop-reposition actions remain at L1 (prompt-rewrite only). Actions counted as adapter-executable (L2/L3) in the Human Pass@K evaluation are restricted to reference-frame injection, segment rerendering, temporal smoothing, enhancement/deblocking, and mask- or reference-conditioned prop insertion.
...
Figure~\ref{fig:annotation_interface_appendix} shows the annotation tool used for manual verification in VideoGenDoctor-Bench-v0. The interface provides a sample navigator, side-by-side playback of the original and perturbed videos, temporal evidence marking on a scrubber timeline, structured failure-verification cards with candidate codes and confidence fields, and a free-form notes field for adjudication. Each annotator is assigned a random subset to avoid systematic bias; dual annotation is performed on the reliability subset.
...
Figure~\ref{fig:diagnostic_report} shows a representative diagnostic report produced by the default VideoGenDoctor pipeline for a perturbed video from the controlled fixture. The report includes global quality and alignment scores, a ranked list of failure records (each with taxonomy code, confidence, temporal span, affected target, and representative keyframes), derived patch hints, and segment-level evidence summaries. The report is designed to be both machine-parsed by the patch compiler and human-audited by a reviewer inspecting the keyframe evidence.
...
\paragraph{Patch-usefulness rubric.}
Table~\ref{tab:human_rubric} defines the 5-point Likert scale used by raters to assess patch usefulness. Scores 1--2 denote insufficient or non-actionable patches; scores 3--5 denote actionable, effective, and resolution-grade patches. Raters are trained on 20 calibration samples before evaluation; inter-rater calibration is verified on a 10\% pilot subset.
...
\paragraph{No-op and wrong-target analysis.}
Table~\ref{tab:no_op_wrong_target_analysis} examines error modes on ambiguous and real-normal samples where a correct response is often no\_op\_uncertain rather than a concrete patch. VideoGenDoctor-diagnosis achieves the highest correct no-op rate (0.738), over 21 percentage points above VLM structured reporting (0.476). VideoGenDoctor-full achieves the lowest wrong-target patch rate (0.081) and over-confident patch rate (0.164), while VLM structured reporting incurs the highest over-confident patch rate (0.396), indicating that without rule-based gating, VLMs frequently over-commit to concrete patches on ambiguous or normal inputs.
...
\paragraph{Qualitative case-study summary.}
Table~\ref{tab:case_study_summary} summarizes three categories of qualitative case studies. Successful structured repair plans (18/24 majority-judged successful) demonstrate the intended diagnosis-to-repair pathway: a localized evidence span triggers a template-constrained patch that resolves the target failure without introducing new artifacts. Ambiguous cases with appropriate abstention (16/24) show that no\_op\_uncertain avoids over-repair when the evidence is insufficient for a concrete patch. Over-repair and wrong-target failures (7/24 majority-judged successful) represent the primary failure mode: a concrete patch targets the wrong object or temporal span, typically when overlapping artifacts confuse the rule engine.
...
\paragraph{Leave-one-code-family-out rule ablation.}
Table~\ref{tab:leave_one_code_family} evaluates what happens when an entire code family's dedicated rules are disabled, leaving only generic embedding drift, optical-flow anomaly, and structured VLM verification. Average macro-F1 drops substantially to 0.774 (from 0.911), confirming that family-specific detectors carry most of the diagnostic signal. Motion-family removal causes the largest degradation (macro-F1 0.721), consistent with optical flow being the single most critical signal. Scene-family removal (macro-F1 0.748) and camera-family removal (0.754) follow. Style-family removal has the mildest impact (0.836), reflecting the relatively strong contribution of generic CLIP embedding drift to style/compression artifact detection.
...
\paragraph{Generator configuration metadata.}
Table~\ref{tab:generator_config} reports the exact generator configuration parameters used to produce the real-failure and real-normal subsets. For each generator, the table records the checkpoint identifier, fps, frame count, resolution, sampling steps, guidance scale, scheduler, seed range, and reference source or prompt rule. CogVideoX and Wan use the same ShotIR-to-prompt template (v1); Stable Video Diffusion uses reference frames extracted from ShotIR character anchors; HunyuanVideo uses a multi-shot prompt template (v1). When a configuration field is not exposed by a generator, the corresponding manifest value is set to \texttt{null} rather than omitted.
